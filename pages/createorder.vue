<template>
  <LoadingGroup :pending="pending" :error="error">
    <n-card class="createorder">
      ...
      <div class="flex mb-5">
        <n-image
          class="rounded flex-shrink-0 w-[180px] h-[100px]"
          :src="data.cover"
        ></n-image>
      ...
      <n-grid :x-gap="20" :cols="4" class="mb-3">
        <n-grid-item v-for="(item, index) in couponData.rows" :key="index">
          <n-button 
            ...
            strong 
            secondary 
            :type="user_coupon_id.value === item.id ? 'warning' : 'tertiary'"
          >
            ￥{{item.price}} 优惠卷
          </n-button>
        </n-grid-item>
      ...
      <div v-if="couponData.count == 0" class="text-gray-400 text-sm mb-5">暂无优惠卷</div>
      <h4>支付方式</h4>
      <div class="flex mb-5">
        <span class="wxpay">
      ...
      <div class="flex items-center mb-5">
        ...
        <span v-if="user_coupon_id">
          优惠卷已抵扣: <b class="text-red-500">{{coupon_price}}</b> 元, 
        </span>
...
const price = computed(() => {
  const p = ((data.value.price * 1000 - coupon_price.value * 1000) / 1000).toFixed(2)
  return p <= 0 ? 0 : p
})
...
const submit = async () => {
  ...
  const {data: createOrderResult } = await useCreateOrderApi(d)
  loading.value = false
