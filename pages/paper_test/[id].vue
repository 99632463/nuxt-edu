      ...
      <n-card>
        ...
        <n-grid :x-gap="12" :cols="4">
          <n-grid-item v-for="(item,index) in testpaper_questions" :key="index">
            <n-tag @click="scrollToDom(index)" :type="item.isTest ? 'success': ''" class="mb-2 w-full cursor-pointer flex items-center justify-center">{{index+1}}</n-tag>
          </n-grid-item>
        </n-grid>
        <n-divider></n-divider>
        <div>
...
<script setup>
import {NGrid,NGridItem,NButton,NTag,NCard,NDivider} from 'naive-ui'
...
// 滚动到指定位置
const scrollToDom = i => {
  const dom = document.getElementById('question_' + i)
